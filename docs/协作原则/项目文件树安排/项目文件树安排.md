...
以下两个文件是两人共同参考的主方案，也是当前项目的权威来源：

| 主方案 | 作用 | 变更要求 |
| --- | --- | --- |
| `docs/方案/AI基础教育公益网站搭建方案_V0.2.docx` | 产品定位、范围、课程主线、页面和阶段基线 | 建立独立 Issue，指定唯一编辑者，成员 A/B 均批准后合并 |
| `docs/协作原则/项目协作/两人协作与分阶段部署方案_V0.2.md` | 分工、Git、PR、部署、安全和协作基线 | 建立独立 Issue，指定唯一编辑者，成员 A/B 均批准后合并 |

本文是共享的实施目录契约，`环境要求/环境要求.md` 是共享环境参考；它们不是个人修改方案，也不得放入成员个人目录。

共同文件遵守：

1. `main` 中已合并的版本始终生效；个人提案、未合并分支和 PR 评论不能静默覆盖主方案。
2. 同一个共同文件同一时间只允许一名指定编辑者修改，另一人通过 review 提出意见。
3. 路由、阶段范围、完成语义、角色分工、schema、安全或部署边界发生变化时，先合并独立的共享方案或接口 PR，再开始依赖它的实现。
4. 修改 DOCX 时必须独占编辑；PR 同时提供 Markdown 变更摘要，列出受影响章节、修改前后语义和迁移影响。
5. 纯错字、链接和不改变含义的排版修复可以走普通 PR，但不能顺带改变规则。

### 0.2 个人修改方案与修改日志

`main` 需要保留双方的个人记录，便于追踪决策和工作，但每个人只直接编辑自己的目录：
...
这里的“成员 A/B”是固定协作者标识，不随技术、产品或内容职责调整而交换。

| 目录 | 所有者 | 内容性质 | 对方如何参与 |
| --- | --- | --- | --- |
| `docs/修改方案/成员A/` | 成员 A | A 提出的变更方案，批准前不生效 | 成员 B 在 Issue/PR 中评审，不直接改文件 |
| `docs/修改方案/成员B/` | 成员 B | B 提出的变更方案，批准前不生效 | 成员 A 在 Issue/PR 中评审，不直接改文件 |
| `docs/修改日志/成员A/` | 成员 A | A 已实际完成的修改、验证和偏差记录 | 成员 B 留评论，由 A 修正 |
| `docs/修改日志/成员B/` | 成员 B | B 已实际完成的修改、验证和偏差记录 | 成员 A 留评论，由 B 修正 |

个人目录规则：

1. 他人不得直接删除、移动、改名、格式化或重写个人方案和日志；发现问题时由所有者在自己的分支修正。
2. 个人修改方案只是提案，不能通过放入个人目录绕过共同主方案、公共契约或另一位成员的审核。
3. 修改方案命名为 `YYYY-MM-DD-Issue-编号-主题_V版本.md`，状态写在文件头部，不通过移动文件表示状态。
4. 修改日志命名为 `YYYY-MM-DD-Issue-编号-主题.md`，一个 Issue 原则上对应一份日志，并在头部记录 PR、commit、实际文件和验证结果；编号尚未产生时写“待创建”，日志与修改位于同一提交时写“本文件所在提交”。
5. 日志只记录已经发生的事实；尚未批准的取舍留在修改方案中。修正历史日志时由原作者追加“更正”说明，不抹去原结论。
6. 个人目录不得保存密钥、未脱敏学生输入、真实身份、学校班级、账号凭据或其他敏感信息。
7. 个人目录是 `docs/` 下按人员命名的唯一例外，不得在 `src/`、`content/`、`data/` 或 `tests/` 中复制成员目录。

Issue #2 是一次性的目录边界初始化，由指定编辑者创建四个目录及说明文件；该规则合并后，个人目录的独占编辑边界正式生效。
...
├── docs/
│   ├── 方案/
│   │   └── AI基础教育公益网站搭建方案_V0.2.docx # 共同主方案：产品与技术基线
│   ├── 协作原则/
│   │   ├── 项目协作/
│   │   │   └── 两人协作与分阶段部署方案_V0.2.md # 共同主方案：协作与部署基线
│   │   ├── 项目文件树安排/
│   │   │   └── 项目文件树安排.md               # 共享目录契约
│   │   └── 环境要求/
│   │       └── 环境要求.md                     # 共享环境参考
...
目录所有权用于明确主责与审核路由，通常不代表只有主责人能修改。唯一例外是 `docs/修改方案/成员A|成员B/` 和 `docs/修改日志/成员A|成员B/`：个人目录只由对应成员直接编辑。
...
| 两份共同主方案 | 产品与协作权威基线 | 每次指定唯一编辑者 | 成员 A/B 均批准 |
| `docs/协作原则/项目文件树安排/` | 共享目录、所有权和依赖契约 | 每次指定唯一编辑者 | 成员 A/B 均批准 |
| `docs/协作原则/环境要求/` | 共享开发环境参考 | 按 Issue 指定 | 另一位成员 |
| `docs/修改方案/成员A/`、`docs/修改日志/成员A/` | 成员 A 的个人提案和事实记录 | 仅成员 A | 成员 B 通过 review |
| `docs/修改方案/成员B/`、`docs/修改日志/成员B/` | 成员 B 的个人提案和事实记录 | 仅成员 B | 成员 A 通过 review |
...
| A 的个人方案与日志 | B 的个人方案与日志 | 双方不直接修改对方个人目录 |
...
- PR 描述必须列出写入目录、公共契约影响、需要哪位主责审核、验证命令、桌面/移动端截图和待审核内容。
- 个人方案或日志可以随对应任务 PR 合并，但只能写入作者自己的目录；不得顺带修改另一人的个人记录。
- 修改共同主方案或共享协作规则时，使用独立的方案 PR，不与可独立验收的功能代码混合。
- 两个变化如果能独立验收就拆成两个 PR；不能拆分时指定一个负责人，并由所有受影响目录的主责审核。
...
- 文件按职责拆分，工程目录不按人员命名；仅 `docs/修改方案/` 和 `docs/修改日志/` 允许固定的 `成员A/`、`成员B/` 所有权目录。
- 禁止在其他位置建立 `member-a/`、`member-b/`、`temp/`、`new/`、`copy/` 和 `final-v2/` 这类目录或文件。
...
- [ ] 个人修改方案和日志只写入 PR 作者自己的成员目录，没有改动另一人的个人记录。
- [ ] 共同主方案或共享协作规则的语义变更已指定唯一编辑者，并得到另一位成员明确批准。
